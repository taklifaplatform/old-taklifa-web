...
class CartItemResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        if (in_array('Webkul\Marketplace\Providers\MarketplaceServiceProvider', config('app.providers'))) {
            $shopTitle = ! empty($this->resource->additional['seller_info'])
                ? app('Webkul\Marketplace\Repositories\SellerRepository')
                    ->find($this->resource->additional['seller_info']['seller_id'])['shop_title']
                : null;
        }
