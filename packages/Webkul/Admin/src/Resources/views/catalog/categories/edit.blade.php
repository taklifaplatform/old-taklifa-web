<x-admin::layouts>
    ...
    <!-- Category Edit Form -->
    <x-admin::form :action="route('admin.catalog.categories.update', $category->id)" enctype="multipart/form-data" method="PUT">

        {!! view_render_event('bagisto.admin.catalog.categories.edit.edit_form_controls.before', [
            'category' => $category,
        ]) !!}
        ...
        <div class="flex gap-2.5 mt-3.5 max-xl:flex-wrap">
            ...
            <div class="flex flex-col gap-2 flex-1 max-xl:flex-auto">
                ...
                <div class="p-4 bg-white dark:bg-gray-900 rounded box-shadow">
                    ...
                    <x-admin::form.control-group>
                        ...
                        <v-field type="text" name="{{ $currentLocale->code }}[name]"
                            value="{{ old($currentLocale->code)['name'] ?? ($category->translate($currentLocale->code)['name'] ?? '') }}"
                            label="{{ trans('admin::app.catalog.categories.edit.name') }}" rules="required"
                            v-slot="{ field }">
                            <input type="text" name="{{ $currentLocale->code }}[name]"
                                id="{{ $currentLocale->code }}[name]" v-bind="field"
                                :class="[errors['{{ $currentLocale->code }}[name]'] ?
                    ...
                    @if ($categories->count())
                        <div>
                            ...
                            <div class="flex flex-col gap-3">
                                <x-admin::tree.view input-type="radio" name-field="parent_id" value-field="id"
                                    id-field="id" :items="json_encode($categories)" :value="json_encode($category->parent_id)" :fallback-locale="config('app.fallback_locale')">
                                </x-admin::tree.view>
                            </div>
                        </div>
                    @endif
                ...
                <div class="p-4 bg-white dark:bg-gray-900 rounded box-shadow">
                    ...
                    <v-description v-slot="{ isDescriptionRequired }">
                        <x-admin::form.control-group>
                            ...
                            <x-admin::form.control-group.control type="textarea" id="description" class="description"
                                name="{{ $currentLocale->code }}[description]" ::rules="{ 'required': isDescriptionRequired }" :value="old($currentLocale->code)['description'] ??
                                    ($category->translate($currentLocale->code)['description'] ?? '')"
                                :label="trans('admin::app.catalog.categories.edit.description')" :tinymce="true" :prompt="core()->getConfigData(
                                    'general.magic_ai.content_generation.category_description_prompt',
                                )" />
                    ...
                    <div class="flex pt-5">
                        ...
                        <div class="flex flex-col gap-2 w-2/5">
                            ...
                            <x-admin::media.images name="logo_path" :uploaded-images="$category->logo_path
                                ? [['id' => 'logo_path', 'url' => $category->logo_url]]
                                : []" />
                        </div>
                        ...
                        <div class="flex flex-col gap-2 w-3/5">
                            ...
                            <x-admin::media.images name="banner_path" :uploaded-images="$category->banner_path
                                ? [['id' => 'banner_path', 'url' => $category->banner_url]]
                                : []" width="220px" />
                        </div>
                    </div>
                </div>

                {!! view_render_event('bagisto.admin.catalog.categories.edit.card.description_images.after', [
                    'category' => $category,
                ]) !!}
                ...
                <div class="p-4 bg-white dark:bg-gray-900 rounded box-shadow">
                    ...
                    <div class="mt-8">
                        ...
                        <x-admin::form.control-group>
                            ...
                            <x-admin::form.control-group.control type="text" id="meta_title"
                                name="{{ $currentLocale->code }}[meta_title]" :value="old($currentLocale->code)['meta_title'] ??
                                    ($category->translate($currentLocale->code)['meta_title'] ?? '')" :label="trans('admin::app.catalog.categories.edit.meta-title')"
                        ...
                        <x-admin::form.control-group>
                            ...
                            <v-field type="text" name="{{ $currentLocale->code }}[slug]" rules="required"
                                value="{{ old($currentLocale->code)['slug'] ?? ($category->translate($currentLocale->code)['slug'] ?? '') }}"
                                label="{{ trans('admin::app.catalog.categories.edit.slug') }}" v-slot="{ field }">
                                <input type="text" id="{{ $currentLocale->code }}[slug]"
                                    name="{{ $currentLocale->code }}[slug]"
                                    :class="[errors['{{ $currentLocale->code }}[slug]'] ?
                                        'border border-red-600 hover:border-red-600' : ''
                                    ]"
                                    class="flex w-full min-h-[39px] py-2 px-3 border rounded-md text-sm text-gray-600 dark:text-gray-300 transition-all hover:border-gray-400 dark:hover:border-gray-400 focus:border-gray-400 dark:focus:border-gray-400 dark:bg-gray-900 dark:border-gray-800"
                                    v-bind="field" placeholder="{{ trans('admin::app.catalog.categories.edit.slug') }}"
                        ...
                        <x-admin::form.control-group>
                            ...
                            <x-admin::form.control-group.control type="text"
                                name="{{ $currentLocale->code }}[meta_keywords]" :value="old($currentLocale->code)['meta_keywords'] ??
                                    ($category->translate($currentLocale->code)['meta_keywords'] ?? '')" :label="trans('admin::app.catalog.categories.edit.meta-keywords')"
                        ...
                        <x-admin::form.control-group class="!mb-0">
                            ...
                            <x-admin::form.control-group.control type="textarea" id="meta_description"
                                name="{{ $currentLocale->code }}[meta_description]" :value="old($currentLocale->code)['meta_description'] ??
                                    ($category->translate($currentLocale->code)['meta_description'] ?? '')"
            ...
            <div class="flex flex-col gap-2 w-[360px] max-w-full">
                ...
                <x-admin::accordion>
                    ...
                    <x-slot:content>
                        ...
                        <x-admin::form.control-group>
                            ...
                            <x-admin::form.control-group.control type="text" name="position" rules="required"
                                :value="old('position') ?: $category->position" :label="trans('admin::app.catalog.categories.edit.position')" :placeholder="trans('admin::app.catalog.categories.edit.enter-position')" />
                ...
                {!! view_render_event('bagisto.admin.catalog.categories.edit.card.accordion.filterable_attributes.before', [
                    'category' => $category,
                ]) !!}
                    ...
                    <x-slot:content>
                        @foreach ($attributes as $attribute)
                            <x-admin::form.control-group class="flex gap-2.5 items-center !mb-2 last:!mb-0">
                                <x-admin::form.control-group.control type="checkbox" :id="$attribute->name ?? $attribute->admin_name"
                                    name="attributes[]" rules="required" :value="$attribute->id" :for="$attribute->name ?? $attribute->admin_name"
                                    :label="trans('admin::app.catalog.categories.edit.filterable-attributes')" :checked="in_array($attribute->id, $selectedaAtributes)" />
                ...
                {!! view_render_event('bagisto.admin.catalog.categories.edit.card.accordion.filterable_attributes.after', [
                    'category' => $category,
                ]) !!}


                {!! view_render_event('bagisto.admin.catalog.categories.edit.card.accordion.tags.before', [
                    'category' => $category,
                ]) !!}
                        ...
                        @foreach ($tags as $tag)
                            <x-admin::form.control-group class="flex gap-2.5 items-center !mb-2 last:!mb-0">
                                <x-admin::form.control-group.control type="checkbox" :id="$tag->name ?? $tag->admin_name" name="tags[]"
                                    :value="$tag->id" :for="$tag->name ?? $tag->admin_name" :label="trans('admin::app.catalog.tags.edit.tags')"
                ...
                {!! view_render_event('bagisto.admin.catalog.categories.edit.card.accordion.tags.after', [
                    'category' => $category,
                ]) !!}
        ...
        {!! view_render_event('bagisto.admin.catalog.categories.edit.edit_form_controls.after', [
            'category' => $category,
        ]) !!}
