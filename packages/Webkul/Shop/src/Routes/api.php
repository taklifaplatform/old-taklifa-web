...
use Webkul\Shop\Http\Controllers\API\QuoteController;
...
Route::group(['middleware' => ['locale', 'theme', 'currency'], 'prefix' => 'api'], function () {
    ...
    Route::controller(QuoteController::class)->prefix('checkout/quote')->group(function () {
        Route::get('', 'index')->name('shop.api.checkout.quote.index');

        Route::post('', 'store')->name('shop.api.checkout.quote.store');

        Route::put('', 'update')->name('shop.api.checkout.quote.update');

        Route::delete('', 'destroy')->name('shop.api.checkout.quote.destroy');

        Route::delete('selected', 'destroySelected')->name('shop.api.checkout.quote.destroy_selected');

        Route::post('move-to-wishlist', 'moveToWishlist')->name('shop.api.checkout.quote.move_to_wishlist');

        Route::post('coupon', 'storeCoupon')->name('shop.api.checkout.quote.coupon.apply');

        Route::delete('coupon', 'destroyCoupon')->name('shop.api.checkout.quote.coupon.remove');
